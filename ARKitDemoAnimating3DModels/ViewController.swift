...
class ViewController: UIViewController {
    ...
    @objc func addShipToSceneView(withGestureRecognizer recognizer: UIGestureRecognizer) {
        ...
        guard let shipScene = SCNScene(named: "models.scn"),
            let shipNode = shipScene.rootNode.childNode(withName: "stone", recursively: false)
            else { return }
        
        var actionRotate = SCNAction.rotateBy(x: 1.25, y: 1, z: 1.5, duration: 8)
            actionRotate = SCNAction.repeatForever(actionRotate)
        
        var actionMoveUp = SCNAction.moveBy(x: 0, y: 0.5, z: 0, duration: 8)
        ...
        //shipNode.geometry?.materials.first?.transparency = 0.5
